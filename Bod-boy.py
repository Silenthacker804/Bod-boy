...
def login_lagi334():
 ...
 try:
  ...
  tokenw = open(".token.txt", "w").write(toke)
  cokiew = open(".cok.txt", "w").write(cookie)
  print('\nlogin berhasil , selanjutnya di arahkan ke menu !');time.sleep(2);os.system('clear')
  
  menu()
  
 except Exception as e:
  os.system('rm -rf .cok.txt && rm -rf .token.txt');print(e);time.sleep(2);os.system('clear')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r└──{sir}[RIFAN]{x} ?? {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
